<template>
  <div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    <div class="section-title" :style='{"padding":"50px 0 0","margin":"0px 0","color":"#000","textAlign":"center","background":"url(http://codegen.caihongy.cn/20230510/6e792688c5fc46fc8bd7b19893e0186f.png) no-repeat center top","fontSize":"30px","lineHeight":"54px","fontWeight":"bold"}'>确认下单</div>
                ...
                this.$http.get(`${item.tablename}/info/${item.goodid}`).then(res => {
                    ...
                    this.$http.post(`${item.tablename}/update`, data).then(res => {
                        ...
                        this.$http.post('orders/add', order).then(res => {
                            ...
                            // 如果该商品存在积分
                            if (data.jf) {
                                this.user.jf = parseInt(this.user.jf) + parseInt(item.price * item.buynumber);
                            }
                ...
                this.$http.get(`${item.tablename}/info/${item.goodid}`).then(res => {
                    ...
                    this.$http.post(`${item.tablename}/update`, data).then(res => {
                        ...
                        this.$http.post('orders/add', order).then(res => {
                            ...
                            // 如果该商品存在积分
                            if (data.jf) {
                                this.user.jf = parseInt(this.user.jf) + parseInt(data.jf * item.buynumber);
                            }
                            this.user.money = this.user.money - item.price * item.buynumber;
                            // 更新用户余额
                            this.$http.post(`${this.userTableName}/update`, this.user).then(res => {
                ...
                this.$http.get(`${item.tablename}/info/${item.goodid}`).then(res => {
                    ...
                    this.$http.post(`${item.tablename}/update`, data).then(res => {
                        ...
                        this.$http.post('orders/add', order).then(res => {
                            // 减少余额，更新订单状态
                            // 判断余额是否充足
                            if (parseInt(this.user.jf) < parseInt(item.price * item.buynumber)) {
                                this.$message({
                                    message: '积分不足',
                                    type: 'error',
                                    duration: 1500
